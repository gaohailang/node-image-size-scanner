...
function NodeImageSizeScanner (options) {
    ...
    this.ignore_image_errors = options.byte_threshold || false;
...
NodeImageSizeScanner.prototype.check = function(options) {
    ...
    request(url, function (err, response, body){
        ...
        images.forEach(function(image){
            var imgsrc = image.attribs.src || image.attribs['data-src'];
            if (imgsrc) {
                var image_url = imgsrc,
...
NodeImageSizeScanner.prototype.processImage = function (image_url, byte_threshold) {
    ...
    request.head(encodeURI(image_url), function(err, res){
        var data = {
            url             : image_url,
        ...
        if (res){
            if (res.headers['content-length']){
                ...
                if (res.statusCode !== 200) {
                    ...
                    data.error = "Unknown Error (" + res.statusCode + ")";
                }
